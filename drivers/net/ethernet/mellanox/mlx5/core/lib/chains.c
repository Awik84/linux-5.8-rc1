...
static const struct rhashtable_params chain_params = {
	.head_offset = offsetof(struct chain, node),
	.key_offset = offsetof(struct chain, chain),
	.key_len = sizeof_field(struct chain, chain),
...
bool mlx5_chains_prios_supported(struct mlx5_chains_priv *chains)
{
	return chains->flags & MLX5_CHAINS_AND_PRIOS_SUPPORTED;
}

static bool mlx5_chains_ignore_flow_level_supported(struct mlx5_chains_priv *chains)
{
	return chains->flags & MLX5_CHAINS_IGNORE_FLOW_LEVEL_SUPPORTED;
}

bool mlx5_chains_backwards_supported(struct mlx5_chains_priv *chains)
{
	return mlx5_chains_prios_supported(chains) &&
	       mlx5_chains_ignore_flow_level_supported(chains);
}

u32 mlx5_chains_get_chain_range(struct mlx5_chains_priv *chains)
{
...
u32 mlx5_chains_get_nf_ft_chain(struct mlx5_chains_priv *chains)
{
	return mlx5_chains_get_chain_range(chains) + 1;
}

u32 mlx5_chains_get_prio_range(struct mlx5_chains_priv *chains)
{
	if (!mlx5_chains_prios_supported(chains))
		return 1;
...
static unsigned int mlx5_chains_get_level_range(struct mlx5_chains_priv *chains)
{
	if (mlx5_chains_ignore_flow_level_supported(chains))
		return UINT_MAX;
...
void
mlx5_chains_set_end_ft(struct mlx5_chains_priv *chains,
		       struct mlx5_flow_table *ft)
{
	tc_end_ft(chains) = ft;
}
...
static int
mlx5_chains_get_avail_sz_from_pool(struct mlx5_chains_priv *chains,
				   int desired_size)
{
	int i, found_i = -1;
...
static void
mlx5_chains_put_sz_to_pool(struct mlx5_chains_priv *chains, int sz)
{
	int i;
...
static void
mlx5_chains_init_sz_pool(struct mlx5_chains_priv *chains, u32 ft_max)
{
	int i;
...
mlx5_chains_create_table(struct mlx5_chains_priv *chains,
...
static void
mlx5_chains_destroy_table(struct mlx5_chains_priv *chains,
			  struct mlx5_flow_table *ft)
{
...
static int
create_chain_restore(struct chain *chain)
{
	struct mlx5_eswitch *esw = chain->chains->dev->priv.eswitch;
	char modact[MLX5_UN_SZ_BYTES(set_add_copy_action_in_auto)];
	struct mlx5_chains_priv *chains = chain->chains;
	...
	if (chains->ns == MLX5_FLOW_NAMESPACE_FDB) {
		 ...
		 * so we write the chain tag directly to reg_b.
		 */
...
static void destroy_chain_restore(struct chain *chain)
{
	struct mlx5_chains_priv *chains = chain->chains;
...
static struct chain *
mlx5_chains_create_chain(struct mlx5_chains_priv *chains, u32 chain)
{
	struct chain *chain_s = NULL;
	int err;
...
static void
mlx5_chains_destroy_chain(struct chain *chain)
{
	struct mlx5_chains_priv *chains = chain->chains;
...
static struct chain *
mlx5_chains_get_chain(struct mlx5_chains_priv *chains, u32 chain)
{
	struct chain *chain_s;
...
mlx5_chains_add_miss_rule(struct chain *chain,
			  struct mlx5_flow_table *ft,
			  struct mlx5_flow_table *next_ft)
{
	struct mlx5_chains_priv *chains = chain->chains;
...
static void
mlx5_chains_put_chain(struct chain *chain)
{
	if (--chain->ref == 0)
...
static struct prio *
mlx5_chains_create_prio(struct mlx5_chains_priv *chains,
			u32 chain, u32 prio, u32 level)
{
...
static void
mlx5_chains_destroy_prio(struct mlx5_chains_priv *chains,
			 struct prio *prio)
{
	struct chain *chain = prio->chain;
...
mlx5_chains_get_table(struct mlx5_chains_priv *chains, u32 chain, u32 prio,
		      u32 level)
{
...
void
mlx5_chains_put_table(struct mlx5_chains_priv *chains, u32 chain, u32 prio,
		      u32 level)
{
	struct prio *prio_s;
	struct prio_key key;
...
struct mlx5_flow_table *
mlx5_chains_get_tc_end_ft(struct mlx5_chains_priv *chains)
{
	return tc_end_ft(chains);
}

struct mlx5_flow_table *
mlx5_chains_create_global_table(struct mlx5_chains_priv *chains)
{
...
void
mlx5_chains_destroy_global_table(struct mlx5_chains_priv *chains,
				 struct mlx5_flow_table *ft)
{
	mlx5_chains_destroy_table(chains, ft);
}

static struct mlx5_chains_priv *
mlx5_chains_init(struct mlx5_core_dev *dev, u32 flags, u32 ft_max,
		 struct mlx5_flow_table *default_ft,
		 u32 max_restore_tag, u32 group_num,
		 enum mlx5_flow_namespace_type ns)
{
	struct mlx5_chains_priv *chains_priv;
	...
	mlx5_core_dbg(dev,
		      ...
		      max_flow_counter, group_num, ft_max);

	chains_priv->dev = dev;
	chains_priv->flags = flags;
	chains_priv->ns = ns;
	chains_priv->group_num = group_num;
	tc_default_ft(chains_priv) = tc_end_ft(chains_priv) = default_ft;
	...
	mlx5_chains_init_sz_pool(chains_priv, ft_max);
	...
	mapping = mapping_create(sizeof(u32), max_restore_tag,
				 true);
	if (IS_ERR(mapping)) {
		err = PTR_ERR(mapping);
		goto mapping_err;
...
static void
mlx5_chains_cleanup(struct mlx5_chains_priv *chains)
{
...
struct mlx5_chains_priv *
mlx5_chains_create(struct mlx5_core_dev *dev, u32 flags, u32 ft_max,
		   struct mlx5_flow_table *default_ft,
		   u32 max_restore_tag, u32 group_num,
		   enum mlx5_flow_namespace_type ns)
{
	...
	chains = mlx5_chains_init(dev, flags, ft_max,
				  default_ft, max_restore_tag,
				  group_num, ns);
...
void
mlx5_chains_destroy(struct mlx5_chains_priv *chains)
{
	mlx5_chains_cleanup(chains);
}

int
mlx5_chains_get_chain_mapping(struct mlx5_chains_priv *chains, u32 chain,
			      u32 *chain_mapping)
{
...
int
mlx5_chains_put_chain_mapping(struct mlx5_chains_priv *chains, u32 chain_mapping)
{
	return mapping_remove(chains_mapping(chains), chain_mapping);
}

int mlx5_get_chain_for_tag(struct mlx5_chains_priv *chains, u32 tag,
			   u32 *chain)
{
	int err;

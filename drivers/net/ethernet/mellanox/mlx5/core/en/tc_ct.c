...
static enum mlx5_flow_namespace_type
mlx5_tc_get_ct_ns(struct mlx5_tc_ct_priv *ct_priv)
{
	return esw_offloads_mode(ct_priv->esw) ? MLX5_FLOW_NAMESPACE_FDB :
...
mlx5_tc_ct_get_ct_priv(struct mlx5e_priv *priv)
{
	...
	if (esw_offloads_mode(esw)) {
		...
		return priv->fs.tc.ct_priv;
	}
}

static struct mlx5_flow_handle *
mlx5_tc_ct_rule_insert(struct mlx5_tc_ct_priv *ct_priv,
...
static void
mlx5_tc_ct_rule_delete(struct mlx5_tc_ct_priv *ct_priv,
		       struct mlx5_flow_handle *rule,
		       struct mlx5_flow_attr *attr)
{
	struct mlx5_eswitch *esw = ct_priv->esw;
			       ...
			       u8 zone_restore_id)
{
	 ...
	 * reg_b copy uppon miss.
	 */
	if (!esw_offloads_mode(ct_priv->esw)) {
		err = mlx5e_tc_match_to_reg_set(esw->dev, mod_acts,
			    ...
			    struct mlx5e_tc_mod_hdr_acts *mod_acts)
{
	...
	flow_action_for_each(i, act, flow_action) {
		...
		case FLOW_ACTION_MANGLE: {
			err = alloc_mod_hdr_actions(mdev, mlx5_tc_get_ct_ns(ct_priv),
						    mod_acts);
			if (err)
				return err;
			  ...
			  bool nat, u8 zone_restore_id)
{
	...
	zone_rule->rule = mlx5_tc_ct_rule_insert(ct_priv, spec, attr);
				  ...
				  bool nat)
{
	...
	mod_hdr = mlx5_modify_header_alloc(dev, mlx5_tc_get_ct_ns(ct_priv),
			...
			bool nat)
{
	...
	ns_type = mlx5_tc_get_ct_ns(ct_priv);
	...
	ft_attr.prio =  ns_type ==  MLX5_FLOW_NAMESPACE_FDB ?
			  ...
			  struct mlx5_flow_handle **flow_rule)
{
	...
	ex_attr_sz = attr_sz - sizeof(*attr);
	ct_flow->pre_ct_attr = mlx5_alloc_flow_attr(ex_attr_sz);
	ct_flow->post_ct_attr = mlx5_alloc_flow_attr(ex_attr_sz);
	if (!ct_flow->pre_ct_attr || !ct_flow->post_ct_attr) {
		err = -ENOMEM;
		goto err_alloc_attr;
	}
	 ...
	 * miss.
	 */
	err = mlx5_chains_get_chain_mapping(chains, attr->chain,
					    &chain_mapping);
	if (err) {
	...
	mod_hdr = mlx5_modify_header_alloc(priv->mdev, mlx5_tc_get_ct_ns(ct_priv),
					   pre_mod_acts.num_actions,
					   pre_mod_acts.actions);
	if (IS_ERR(mod_hdr)) {
		err = PTR_ERR(mod_hdr);
	...
	rule = mlx5_tc_ct_rule_insert(ct_priv, post_ct_spec,
	...
	ct_flow->pre_ct_rule = mlx5_tc_ct_rule_insert(ct_priv, orig_spec,
						      pre_ct_attr);
...
err_insert_orig:
	mlx5_tc_ct_rule_delete(ct_priv, ct_flow->post_ct_rule,
			       ct_flow->post_ct_attr);
err_insert_post_ct:
	mlx5_modify_header_dealloc(priv->mdev, pre_ct_attr->modify_hdr);
err_mapping:
	dealloc_mod_hdr_actions(&pre_mod_acts);
	mlx5_chains_put_chain_mapping(chains, ct_flow->chain_mapping);
err_get_chain:
				...
				struct mlx5_flow_handle **flow_rule)
{
	...
	ex_attr_sz = attr_sz - sizeof(*attr);
	pre_ct_attr = mlx5_alloc_flow_attr(ex_attr_sz);
	if (!pre_ct_attr) {
		err = -ENOMEM;
		goto err_attr;
	}
	...
	mod_hdr = mlx5_modify_header_alloc(esw->dev, mlx5_tc_get_ct_ns(ct_priv),
					   mod_acts->num_actions,
					   mod_acts->actions);
	if (IS_ERR(mod_hdr)) {
		err = PTR_ERR(mod_hdr);
	...
	rule = mlx5_tc_ct_rule_insert(ct_priv, orig_spec,
				      pre_ct_attr);
	if (IS_ERR(rule)) {
		err = PTR_ERR(rule);
			...
			struct mlx5e_tc_mod_hdr_acts *mod_hdr_acts)
{
	...
	if (clear_action)
		err = __mlx5_tc_ct_flow_offload_clear(priv, flow, spec, attr, attr_sz,
						      mod_hdr_acts, &rule);
	else
		err = __mlx5_tc_ct_flow_offload(priv, flow, spec, attr, attr_sz,
						&rule);
			 ...
			 struct mlx5_ct_flow *ct_flow)
{
	...
	mlx5_tc_ct_rule_delete(ct_priv, ct_flow->pre_ct_rule,
			       pre_ct_attr);
	mlx5_modify_header_dealloc(priv->mdev, pre_ct_attr->modify_hdr);
		       ...
		       struct mlx5_flow_attr *attr)
{
	...
	mutex_lock(&ct_priv->control_lock);
	__mlx5_tc_ct_delete_flow(ct_priv, flow, ct_flow);
	mutex_unlock(&ct_priv->control_lock);
}
		...
		struct mlx5_tc_ct_priv **_ct_priv)
{
	...
	dev = priv->mdev;
	err = mlx5_tc_ct_init_check_support(priv, &msg);
	if (err) {
		mlx5_core_warn(dev,
	...
	ct_priv->mod_hdr_tbl = esw_offloads_mode(ct_priv->esw) ?
...
mlx5_tc_ct_clean(struct mlx5_tc_ct_priv **_ct_priv,
		 struct mlx5_chains_priv *chains)
{
	struct mlx5_tc_ct_priv *ct_priv = *_ct_priv;
